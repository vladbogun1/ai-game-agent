...
- UI для запуска, остановки и просмотра логов.
...
Запуск UI (рекомендуется для начала):

```bash
python app.py
```

UI содержит выпадающий список мониторов для выбора, что именно слушать.

Запуск в «сухом» режиме CLI (ничего не нажимает, только логирует действия):
...
Запуск с реальным управлением CLI (ОСТОРОЖНО):
...
Workflow `.github/workflows/build-windows.yml` собирает Windows-экзешник и публикует артефакт:

1. Перейдите в Actions → `build-windows`.
2. Нажмите **Run workflow**.
3. Заберите артефакт `AIGameAgent-windows`, внутри будет `AIGameAgent.exe`.
